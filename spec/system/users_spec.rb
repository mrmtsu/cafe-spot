...
RSpec.describe "Users", type: :system do
  ...
  let!(:other_posts) { create(:post, user: other_user) }
  ...
  describe "プロフィールページ" do
    ...
    context "お気に入り登録/解除" do
      ...
      it "お気に入り一覧ページが期待通り表示されること" do
        ...
        expect(page).not_to have_css ".favorite-posts"
        user.favorite(posts)
        user.favorite(other_posts)
        visit favorites_path
        ...
        expect(page).to have_content other_posts.name
        expect(page).to have_content other_posts.description
        expect(page).to have_content "posted by #{other_user.name}"
        expect(page).to have_link other_user.name, href: user_path(other_user)
        user.unfavorite(other_posts)
        visit favorites_path
    ...
    context "通知生成" do
      ...
      context "自分以外のユーザーの投稿に対して" do
        ...
        it "お気に入り登録によって通知が作成されること" do
          ...
          expect(page).to have_content other_posts.name
          expect(page).to have_content other_posts.description
          expect(page).to have_content other_posts.created_at.strftime("%Y/%m/%d(%a) %H:%M")
        end
      ...
      context "自分の投稿に対して" do
        ...
        it "お気に入り登録によって通知が作成されないこと" do
          ...
          expect(page).not_to have_content posts.name
          expect(page).not_to have_content posts.description
          expect(page).not_to have_content posts.created_at
        end
        ...
        it "コメントによって通知が作成されないこと" do
          ...
          expect(page).not_to have_content other_posts.name
          expect(page).not_to have_content other_posts.description
          expect(page).not_to have_content other_posts.created_at
        end
      end
    end
    ...
    context "リスト登録/解除" do
      ...
      it "リスト一覧ページが期待通り表示され、リストから削除することもできること" do
        visit lists_path
        expect(page).not_to have_css ".list-post"
        user.list(posts)
        posts_2 = create(:post, user: user)
        other_user.list(posts_2)
        visit lists_path
        expect(page).to have_css ".list-post", count: 2
        expect(page).to have_content posts.name
        expect(page).to have_content posts.description
        expect(page).to have_content List.last.created_at.strftime("%Y/%m/%d(%a) %H:%M")
        expect(page).to have_content "このお店を行きたいリストに追加しました。"
        expect(page).to have_content posts_2.name
        expect(page).to have_content posts_2.description
        expect(page).to have_content List.first.created_at.strftime("%Y/%m/%d(%a) %H:%M")
        expect(page).to have_content "#{other_user.name}さんがこのお店に行きたいリクエストをしました。"
        expect(page).to have_link other_user.name, href: user_path(other_user)
        user.unlist(List.first)
        visit lists_path
        expect(page).to have_css ".list-post", count: 1
        expect(page).to have_content posts.name
        find('.unlist').click
        visit lists_path
        expect(page).not_to have_css ".list-post"
